/**
 * Copyright (C) 2000,2001,2002,2003  Josh Coalson,  libFLAC - Free Lossless Audio Codec library (Lookup-Table only) 
 * Copyright (C) 2000, Utilator AB, http://www.utilator.com
 * Copyright (C) 2000-2001 ake Hedman, Eurosource, http://www.eurosource.se
 * Project home: eda.sourceforge.net
 * 
 * This program is free software; you can redistribute it and/or
 * modify it under the terms of the GNU General Public License
 * as published by the Free Software Foundation; either version
 * 2 of the License, or (at your option) any later version.
 * 
 * This program  is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 * 
 * You should have received a copy of the GNU General Public License
 * along with EDA. see the file COPYING.  If not, write to
 * the Free Software Foundation, 59 Temple Place - Suite 330,
 * Boston, MA 02111-1307, USA.
 * 
 * Authors: the EDA Development Team.  Its members are listed in a
 * file named AUTHORS, in the root directory of this distribution.
*/ 

#ifndef __CRC8_H__
#define __CRC8_H__

/**
 * A lookup table for CRC-8, poly = x^8 + x^2 + x^1 + x^0, init = 0.
 */
const unsigned char crc_table[] = {

};
  
/**
 * @brief The CRC8 class can calculate CRC8-checksums.
 */
class CRC8 
{
 public:
    /**
     * @brief Default constructor
     */
 	CRC8();
    /**
     *@brief Destructor
     */
  	~CRC8();
    /**
     * @brief Intialize a new caluclation of CRC8 with 0 als Start.
     */
  	void init();
    /**
     * @brief Returns the result von the last calc.
     * @return 
     */
  	unsigned char get();
    /**
     * @brief Updates the checksum with byte_in
     * @param byte_in Some byte 
     */
  	void calc(unsigned char byte_in);
    /**
     * @brief Calculates the checksum from the buffer.
     * @param buffer The buffer that should be used to calculate the checksum.
     * @param len The length of the buffer.
     * @return The Calculated checksum.
     */
  	char from_array(unsigned char buffer[], unsigned int len);
  	int return_byte_count( void );

 private:
    /**
     * @brief Current checksum.
     */
  	unsigned char m_crc;
    /**
     * @brief From how many bytes was the checksum calculated.
     */
  	int m_byte_count;
	
};

#endif /* !__CRC8_H__ */
